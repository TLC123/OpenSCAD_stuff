...
scenegraph = [opT, [ [1,0, 1], [0, 0, 0], [1, 1, 1] ],
 [ [opT, [ [0, 0, 0], [0, 0, 0], [1, 1, 1] ],
     [ [opU, 0/*round union !!*/, [ 
         [opT, [  [ 0, 0  , 0],   [-54.4881, -1.1109, -21.3758],  [0, 0, 0] ],
...
cell=[[-15,-15,-15],[15,15,15]];
 plst=bound_to_vertex_box(cell);
 //for(i=[0:len(plst)-1])translate(plst[i])text(str(i),size=5);
 echo(cell);
 tet(scenegraph,[plst[0],plst[2],plst[5],plst[7]]);
 tet(scenegraph,[plst[0],plst[2],plst[1],plst[5]]);
 tet(scenegraph,[plst[0],plst[2],plst[3],plst[7]]);
 tet(scenegraph,[plst[7],plst[5],plst[4],plst[0]]);
 tet(scenegraph,[plst[7],plst[5],plst[6],plst[2]]);
 
 module tet(scene,v,vm=[],c=4)
 {
     C=avrg(v);
     eC=cube(C,10,3);
    // echo(C,eC);
 if(eC>len3(C-v[0])*2){}
     else if(eC<=-len3(C-v[0])*2){hull(){
     point(v[0]);
         point(v[1]);
         point(v[2]);
         point(v[3]);
         ...
         p01=lerp(v[0],v[1],0.5);
         p02=lerp(v[0],v[2],0.5);
         p03=lerp(v[0],v[3],0.5);
         p12=lerp(v[1],v[2],0.5);
         p23=lerp(v[2],v[3],0.5);
         p31=lerp(v[3],v[1],0.5);
         tet(scene,[v[0],p01,p02,p03],vm,c-1);
         tet(scene,[v[1],p01,p12,p31],vm,c-1);
         tet(scene,[v[2],p02,p12,p23],vm,c-1);
         tet(scene,[v[3],p03,p23,p31],vm,c-1);
         
         tet(scene,[p01,p23,p02,p03],vm,c-1);
         
         tet(scene,[p01,p23,p03,p31],vm,c-1);
         
         tet(scene,[p01,p23,p31,p12],vm,c-1);
         
         tet(scene,[p01,p23,p12,p02],vm,c-1);
         }else{hull(){
     ...
     e2=cube(v[2],10,3);        
     e3=cube(v[3],10,3);        
         p01=lerp(v[0],v[1],inter(e0,e1));
         p02=lerp(v[0],v[2],inter(e0,e2));
         p03=lerp(v[0],v[3],inter(e0,e3));
         p12=lerp(v[1],v[2],inter(e1,e2));
         p23=lerp(v[2],v[3],inter(e2,e3));
         p31=lerp(v[3],v[1],inter(e3,e1));
     e01=cube(p01,10,3);
     e02=cube(p02,10,3);
     e03=cube(p03,10,3);
     e12=cube(p12,10,3);
     ...
     e31=cube(p31,10,3);
         ...
         if(e2<0)point(v[2]);
         if(e3<0)point(v[3]);    
          if(sign(e0)!=sign(e1))point(p01);    
          if(sign(e0)!=sign(e2))point(p02);    
          if(sign(e0)!=sign(e3))point(p03);    
          if(sign(e1)!=sign(e2))point(p12);    
          if(sign(e2)!=sign(e3))point(p23);    
          if(sign(e3)!=sign(e1))point(p31);    
             
             }}
     }
     
     module point(p){translate(p)cube(1e-5);}
     
      module line(p1,p2)
     {hull(){translate (p1)sphere(1e-5);
         translate (p2)sphere(1e-5);}}
 function bound_to_vertex_box(b)=
 let(
 p0=[b[0].x,b[0].y,b[0].z],
 p1=[b[1].x,b[0].y,b[0].z],
 p2=[b[1].x,b[1].y,b[0].z],
 p3=[b[0].x,b[1].y,b[0].z],

 p6=[b[1].x,b[1].y,b[1].z],
 p7=[b[0].x,b[1].y,b[1].z],
 p4=[b[0].x,b[0].y,b[1].z],
 p5=[b[1].x,b[0].y,b[1].z]
