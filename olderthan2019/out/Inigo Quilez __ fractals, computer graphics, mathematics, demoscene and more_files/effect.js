...
EffectPass.prototype.NewTexture = function( wa, slot, url, buffers, keyboard )
{
    ...
    else if( url.mType=="texture" )
    {
        ...
        texture.image.onload = function()
        {
            ...
            if (url.mSrc == "/presets/tex15.png" || url.mSrc == "/presets/tex17.png" || url.mSrc == "/presets/tex19.png")
...
EffectPass.prototype.Paint_Image = function( vrData, wa, d, time, dtime, mouseOriX, mouseOriY, mousePosX, mousePosY, xres, yres, buffers, keyboard )
{
...
EffectPass.prototype.Paint = function( vrData, wa, da, time, dtime, mouseOriX, mouseOriY, mousePosX, mousePosY, xres, yres, isPaused, bufferID, buffers, keyboard )
{
    if( this.mType==="sound" )
    {
    ...
    {
        this.mRenderer.SetRenderTarget( null );
        this.Paint_Image( vrData, wa, da, time, dtime, mouseOriX, mouseOriY, mousePosX, mousePosY, xres, yres, buffers, keyboard );
        this.mFrame++;
    }
...
Effect.prototype.Paint = function(time, dtime, mouseOriX, mouseOriY, mousePosX, mousePosY, isPaused)
{
    var wa = this.mAudioContext;
    ...
    for( var i=0; i<num; i++ )
    {
        ...
        this.mPasses[i].Paint( vrData, wa, da, time, dtime, mouseOriX, mouseOriY, mousePosX, mousePosY, xres, yres, isPaused, null, this.mBuffers, this.mKeyboard );
    }
    ...
    for( var i=0; i<num; i++ )
    {
        ...
        this.mPasses[i].Paint( vrData, wa, da, time, dtime, mouseOriX, mouseOriY, mousePosX, mousePosY, xres, yres, isPaused, bufferID, this.mBuffers, this.mKeyboard );
    }
...
Effect.prototype.newScriptJSON = function( jobj )
{
    ...
    var bufferPassNum = 0; // hack (see below)
    ...
    for( var j=0; j<numPasses; j++ )
    {
        ...
        {
//            this.mPasses[j].SetOutputs( i, rpass.outputs[i].channel );

            // this should come from the DB when we implement Multipass 2.0. But for now, we hardcode it
            var sid = null;

            if( i===0 && rpass.type==="buffer" )
            {
                sid = bufferPassNum;
                bufferPassNum = bufferPassNum + 1;
            }

            this.mPasses[j].SetOutputs( i, sid );
        }
...
Effect.prototype.exportToJSON = function()
{
    ...
    for( var j=0; j<numPasses; j++ )
    {
        ...
        for( var i = 0; i<4; i++ )
        {
            if( this.mPasses[j].mOutputs[i]==null ) continue;
            result.renderpass[j].outputs.push( { channel: this.mPasses[j].mOutputs[i] } );
        }
