...
detail_random_seed = 71; // [1:1:400]

// How many sides/facets for main perimiter wall
sp=[4,5,6,8,12];
perimiter_sides = sp[rands(0,4,1)[0]]; // [4,5,6,8,12,60]
...
!Island ( perimiter_diameter*rands(0.9,1.5,1)[0]+max_turret_width*rands(0.1,0.5,1)[0], perimiter_height*rands(0.4,1.5,1)[0],rands(-5,5,1)[0],floor(rands(0,2,1)[0]) ,perimiter_sides,1);



 
module Island(r,h,over,moat,sides=4,stairs){
intersection(){ 
    seed = rands(0,100,1)[0];// rseed;

push1=[rands(-perimiter_diameter ,perimiter_diameter,1)[0],rands(-perimiter_diameter,0,1)[0],-h*2];
push2=[rands(-perimiter_diameter ,perimiter_diameter,1)[0],rands(-perimiter_diameter,0,1)[0],-h*2];

translate([0,0,-h])cylinder(r*2,r*2,h*2);// cut it all to size
union(){


steps=h/(people_scale*0.2);
stairstart=[0,-perimiter_diameter/2-4*people_scale,0];
stairend=[0,-r,-h ];

///////////////////// stairs last step outside diff  ///////////////////////////////////

if(stairs==1)color("Grey")translate( stairend)translate([0,0,0])scale([5*people_scale,r/2,2*people_scale])translate([0,0.5,0])cube(1,center=true);
///////////////////// eof stairs last step outside diff  ///////////////////////////////////
...
if(stairs==1)color("Grey")union(){for(i=[-0.2:1/(steps*0.25):0.8]){translate(lerp(stairstart,stairend,i))scale([2.5*people_scale,1.5*people_scale,3*people_scale])translate([-0,-0.5,-0.5])bolder(seed+i*100);}}
///////////////////// stairs positive  ///////////////////////////////////
...
translate([0,-people_scale*.65+over*2,-h*2.2])scale([r*0.95,r*0.95,h*2.66-r*0.01+over])
polyhedron([for(i=[0:l])let(r=rands(0,360,1)[0],t=rands(90,180,1)[0])[sin(r)*cos(t),cos(r)*cos(t),sin(t)]],[for(j=[0:n])[rands(0,l-1,1)[0],rands(0,l-1,1)[0],rands(0,l-1,1)[0]]]);};


}
// castle  and gate footprint
union(){
midr=rands(0,360,1)[0];
color("DarkGreen")hull(){

  translate([0,0,-h*0.05]) rotate([0,0,ps])linear_extrude(1, 0, convexity = 3) offset(r = max_turret_width/3) {
          circle(r/2,$fn=sides);}; // footprint
 translate([0,0,-h*0.6])rotate([0,0,midr])linear_extrude(1, 0, convexity = 3) offset(r = 4) {
...
color("DarkGreen")hull(){
 ...
 translate(push1)rotate([0,0,rands(0,360,1)[0]])linear_extrude(1, 0, convexity = 3) offset(r = 4) {
          circle(r,$fn=sides /2);};
 translate(push2)rotate([0,0,rands(0,360,1)[0]])linear_extrude(1, 0, convexity = 3) offset(r = 4) {
          circle(r,$fn=sides  );};
...
//////////////////////shapes the top of the island to fit the castle
  // negative volume
    ...
    rotate([90,0,0])
    translate([0,(3*people_scale)/2+1,perimiter_diameter/2 -3.5*people_scale])
    linear_extrude(5.5*people_scale)
    special_rectangle(2.01*people_scale, 3*people_scale); 

///////////////////// stairs negative  ///////////////////////////////////
*line(stairstart,stairend);
if(stairs==1)union(){
color("Grey")for(i=[0:1/steps:0.87]){
translate(lerp(stairstart,stairend,i))scale([2.01*people_scale,h,h])translate([-0,-0.3,0.5])cube(1,center=true);}

color("Grey")hull(){for(i=[-0.2:1/(steps*0.25):1]){hull(){
translate ([0,0,1.2*people_scale ]){translate(lerp(stairstart,stairend,i))scale([8*people_scale,h,0.5*people_scale])translate([-0,-0.5,0.5])cube(1,center=true);}
translate ([0,0,1.2*people_scale])translate(lerp(stairstart,stairend,i))scale([2.5*people_scale,h,h])translate([-0,-0.3,0.5])cube(1,center=true);}}}
}
...
///////////////////// end of stairs  ///////////////////////////////////
 ...
 if (moat==1){   color("Grey")  hull(){ translate ([0,0,1.8])rotate([0,0,0])turret_round ( perimiter_diameter+max_turret_width*1.1, 1, false, perimiter_sides*2,0,0); 
    color("Grey") translate ([0,0,1.8+h*2])scale([2,2,1])rotate([0,0,0])turret_round ( perimiter_diameter+max_turret_width*1.1, h, false, perimiter_sides*3,0,0); }}
...
module bolder(bseed)
{
  loops=3;
       ...
       for(loop=[0:loops]){
                ...
                ri=rands(0,360,l,bseed+loop); // NEED TO USE rseed
                t=rands(0,180,l,bseed+loop+1);  // NEED TO USE rseed
