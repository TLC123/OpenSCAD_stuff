...
v1 = 
 [
  for (t = [0: 4]) concat(rndc(t+1) * 100 )
];
v=(v1);
spl = v2spl(v, t);
//ShowControl(v);
spline(spl,40 ,gap=0.2){cube(1,center=true);}  ;
    ...
    for (ii = [start + det: det: stop]) {
      ...
      if ($children > 0) {
        ...
        hull() {
          translate(t(spl2(i, v))) 
 rotate(spl2euler(i, v)) 
scale(spl2(i, v)[3]) rotate([twist * ii, 0, 0]) rotate([0, -90, 0]) sphere(1);
          translate(t(spl2(i - detail * gap, v))) 
rotate(spl2euler(i - detail * gap, v)) 
scale(spl2(i - detail * gap, v)[3]) rotate([twist * (ii - det), 0, 0]) rotate([0, -90, 0]) sphere(1);
        }
      }
    }
  }
