...
spline(spl,d=detail,twist=360*0,gap=gap)
{

linear_extrude(1e-2){
offset(0.4)scale([1,3])rotate( 45)square(1,center=true);}
linear_extrude(1e-2){
offset(0.4)rotate(120)scale([1,3])rotate( 45)square(1,center=true);}

linear_extrude(1e-2){

offset(0.4)rotate(240)scale([1,3])rotate( 45)square(1,center=true);}
}
...
translate(t(spl2(i, v))) rotate(spl2euler(i, v)) 
scale(spl2(i, v)[3]) 
rotate([twist * ii, 0, 0]) 
rotate([0, -90, 0]) 
children(j);
               
translate(t(spl2(i - detail * gap, v))) rotate(spl2euler(i - detail, v)) 
...
//
//function spl2euler(i, v) = [0, -asin(spl2v(i, v)[2]), atan2(spl2xy(spl2v(i, v))[1], spl2xy(spl2v(i, v))[0])];(a,b,unit(cross(a,b))) 
//function anglev(u,v) = acos( dot(u,v) / (mod(u)*mod(v)) );
//echo(anglev([0,1,0],[1,0,0]));
