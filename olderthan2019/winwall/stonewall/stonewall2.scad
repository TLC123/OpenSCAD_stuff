...
module stone(){rotate([rands(-4, 4, 1)[0],rands(-4,4, 1)[0],rands(-4, 4, 1)[0]]) intersection() {
    cube(0.74,center=true);
    resize([1,1,1]) rotate([rands(0, 360, 1)[0],rands(0, 360, 1)[0],rands(0, 360, 1)[0]])resize([rands(3, 10, 1)[0],rands(3, 10, 1)[0],rands(3, 10, 1)[0]])
...
module Wall( length, height)
{
    ...
    translate([-37,-25,0]) cube([74,length*40+44,height*20+25 ]) ;
    ...
    union() {
		...
		for (j = [0:length-1]) {
				color( colors[rands(0, n_colors-1, 1)[0]])
				translate([rands(-10, 10, 1)[0],j*40+rands(-5, 5, 1)[0]+20,(height+.5)*20])
