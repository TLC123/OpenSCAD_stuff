...
{ union(){
    r=rands(-5,15+j,1)[0];
     rv=rands(-10-j,10+j,1)[0];
    goon=rands(0,1,1)[0]+1.1-1/i;
    t=min(max(15,75/i),25);
    if(goon<1){ difference(){
        color(mycolor)hull(){sphere(d=2*i, $fn=12);
        rotate([r,rv,i]) translate([0,0,t])cylinder(d=2*(i-1),h=0.1,$fn=12);}
        
      color("tan")  rotate([r,2+rv,i]) translate([0,0,t-0.1])cylinder(d=1.7*(i-1),h=0.4,$fn=12);}}
        
        else{color(mycolor)hull(){sphere(r=i, $fn=12);
        rotate([r,rv,i]) translate([0,0,t])sphere(d=2*(i-1), $fn=12);}
        
        }
    if(i>=1 && goon>=1){
    rotate([r,rv,i]) translate([0,0,t])grow(i-1,j+1);
    if(rands(0,1+j/15,1)[0]>=1){
    rotate([r,rv,i-90]) translate([0,0,t-10])grow(i-2,0);
    }
        }
...
{color(mycolor) union(){
    r=rands(-15,15,1)[0];
    rv=rands(-0.11,1,1)[0];
    t=(max(10,25/i))!=1/0?max(10,25/i):0;
    hull(){sphere(d=2*i, $fn=12);
    rotate([rv,r,0]) translate([0,0,t])sphere(d=2*(i-1), $fn=12);}
    if(i>=1){
...
scale(0.3)union(){
 ...
 translate([0,0,8])  grow(20,0);
  }

translate([0,0,-0.001])color("DimGray")cylinder(r=100.001,h=5.001);
translate([0,0,5])color("DimGray")cylinder( 100.001, 95, 5);
