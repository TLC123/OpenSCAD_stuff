...
*/
Resolution = 3;//[5:5:50]
MinSubdivisions=2;//[2:1:10] 
// Smaller number larger blob
Thresh1 = 1.2;// [1.5:0.01:2.5]
Diagnostic=false;
Oversize=0.25;
...
size=max(findMax(BallList, 0)-x,findMax(BallList, 1)-y,findMax(BallList,2)-z);

DrawSubMetaballs(BallList,ColorList,Threshold,x-(size*Oversize),y-(size*Oversize),z-(size*Oversize),size+(size*Oversize)); 
...
//function Powerball(x, y, z, Ball) = (Ball[3] / sqrt((x - Ball[0]) * (x - Ball[0]) + (y - Ball[1]) * (y - Ball[1]) + (z - Ball[2]) * (z - Ball[2])));
 //function Powerball(x, y, z, Ball) = min((Ball[3]/abs(x - Ball[0])),(Ball[3]/abs(y - Ball[1])),(Ball[3]/abs(z - Ball[2]))) ;
function Powerball(x, y, z, Ball) = 3*tresh-((Ball[3]/abs(x - Ball[0]))+(Ball[3]/abs(y - Ball[1]))+(Ball[3]/abs(z - Ball[2])))/3 ;
