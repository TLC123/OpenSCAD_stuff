...
{ union(){
    r=rands(-5,15+j,1)[0];
     rv=rands(-(10/i),(10/i),1)[0];
    goon=rands(-0.1,1,1)[0]+1.1-2/i;
    t=min(max(15,75/i),25);
    
    if(goon<0&&j>1){ 
        difference(){
        color(mycolor)hull(){#sphere(d=2*i,h=0.1,$fn=12);
rotate([r,rv,i]) translate([0,0,t])rotate([30,-3,0])cylinder(d=2*(i-1),h=0.1,$fn=12);}
        
      color("tan")  
rotate([r,rv,i]) translate([0,0,t])rotate([30,-3,0])cylinder(d=1.7*(i-1),h=5,$fn=12);}}
    ...
    rotate([r,60+rv,i-90]) translate([0,0,0])grow(i-2,0);
    }
        }
        }
