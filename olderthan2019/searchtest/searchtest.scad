...
// feature detector
function  findp2p(inp,pl,mi,ma,f= 16)=let(p= [rnd(0.001),rnd(0.001),rnd(0.001)]+mima3(inp,mi,ma))
f>0&&len(pl)>0?let(q = avrg([for(i=[0:len(pl)-1]) point2plane( p, pl[i][0] , pl[i][1] ) ]) ,df=q-p)findp2p( (p+(df*1.05  ) ),pl,mi,ma,f-1): p ;
...
//Main evaluator
function eval(p,model )= min(cube(invtransform(p),1),cube(invtransform(p,[-2,0,0]),1));
//////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////
// testing primitives
