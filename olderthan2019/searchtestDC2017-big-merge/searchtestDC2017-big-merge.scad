...
scenegraph = [opT, [ [1,0, 1], [0, 0, 0], [1, 1, 1] ],
 [ [opT, [ [0, 0, 0], [0, 0, 0], [1, 1, 1] ],
     [ [opU, 3/*round union !!*/, [
         [opT, [  [rnd(-20, 20), rnd(-20, 20), 0],  [rnd(-90, 90), rnd(-90, 90), rnd(-90, 90)],  [1, 1, 1] ],
             [  [f_cube, [  15, 5  ]] ]/*atm all hardcoded for cubes*/
         ],
         [opT, [  [rnd(-20, 20), rnd(-20, 20), 0],  [rnd(-90, 90), rnd(-90, 90), rnd(-90, 90)],  [1, 1, 1] ],
             [  [f_sphere, [20]] ]
         ],
...
//////////////////////////////////////////////////////////////////////////////////////
// It starts here
cell = autobound(scenegraph, cubic = false, pad = 0.25);
echo("boundingbox=",cell);
translate([100, 0, 0]) color("Red") showcell(cell);
translate([100, 0, 0]) unglymarch(scenegraph, cell, c = 5);

color("Red") showcell(cell);
 showfield(scenegraph, cell, len3(cell[1] -
 cell[0]) / 100);

translate([-100, 0, 0]) color("Red") showcell(cell);
translate([-100, 0, 0]) DualC(scenegraph, cell , depth = 4);
//////////////////////////////////////////////////////////////////////////////////////
/* Dual Contour 
...
let (filterv = [
 ...
 faces = faceworker(filterv), neighbours = [
 for (i = [0: len(points) - 1])[
 for (j = [0: len(faces) - 1])
 if (faces[j][0] == i || faces[j][1] == i || faces[j][2] ==
 i || faces[j][3] == i) j]
 ], t = relax(points, faces, neighbours, filterv, cell))
t;

function relax(points, faces, neighbours, filterv, cell, c = 3) = c >
 0 ?
 let (midpoints = [
...
f > 0 && len(pl) > 0 ?
 let (q = avrg([
 for (i = [0: len(pl) - 1]) point2plane(p, pl[i][0], pl[i]
 [1])
...
function encode(v) =
let (xseed = (rnd(9999999999999999, -9999999999999999,
 9999999999999999 + v.x * 10000)), yseed = (rnd(9999999999999999, -
 9999999999999999, xseed + v.y * 10000)), zseed = (rnd(
 9999999999999999, -9999999999999999, yseed + v.z * 10000)), hash =
 round(rnd(9999999999999999, 0, zseed)))
hash;
...
opcode == f_cube ? cube(q, 15,0) : //10
 opcode == f_sphere ? cube(q, 12,10) : //10
 opcode == f_torus ? cube(q, 15,5) : //10
 0;
...
function parseopT(q, M, scene) = min([
 for (i = [0: len(scene) - 1]) eval(Transf(q, M), scene[i])
]);
...
// 
//; 
function ChamferBox(op, b = [1, 1, 1] /*[l,w,h]*/ , ch = 0 /*Chamfer*/ ,
 r = 0 /*radius*/ ) =
...
module trender2(t) {
 ...
 color((rndc() + [0.5, 1, 1]) / 2) mupolyhedron(t[0], t[1][i]);
 }
}
...
let (VeryFar = 9999999999999, p1 = vec * VeryFar, p2 = p1 + vec, e1 =
 abs(eval(p1, scene)), e2 = abs(eval(p2, scene)), scale = abs(e2 -
 e1), corrected = (e1 / scale), distance = VeryFar - e1 //distance= VeryFar-corrected
)
