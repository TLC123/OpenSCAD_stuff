...
newfaces=fp_removevertex(removies,points[j],alt=3),
r=newflattness(newfaces),
h=newflattness(removies)
//e=echo(r,h)
) 
r<=h?
    let(keepers=[for(i=[0:len(nn)-1])([points[(faces[nn[i]][0])],points[(faces[nn[i]][1])],points[(faces[nn[i]][2])]  ])])
    fp_glue( concat(keepers,newfaces))
    :
    let(newfacesalt=fp_removevertex(removies,points[j],alt=5),
    ralt=newflattness(newfacesalt))
    ralt<=h?
        let(keepers=[for(i=[0:len(nn)-1])([points[(faces[nn[i]][0])],points[(faces[nn[i]][1])],points[(faces[nn[i]][2])]  ])])
        fp_glue( concat(keepers,newfacesalt))
        
        :
        let(newfacesalt2=fp_removevertex(removies,points[j],alt=7),
        ralt2=newflattness(newfacesalt2))
        ralt2<=h?
            let(keepers=[for(i=[0:len(nn)-1])([points[(faces[nn[i]][0])],points[(faces[nn[i]][1])],points[(faces[nn[i]][2])]  ])])
            fp_glue( concat(keepers,newfacesalt2))
            
            :
            let(newfacesalt3=fp_removevertex(removies,points[j],alt=11),
            ralt3=newflattness(newfacesalt3))
            ralt3<=h?
                let(keepers=[for(i=[0:len(nn)-1])([points[(faces[nn[i]][0])],points[(faces[nn[i]][1])],points[(faces[nn[i]][2])]  ])])
                fp_glue( concat(keepers,newfacesalt3))
...
function fp_removevertex(faces,vertex,alt=0)=let(
n= avrg([for(i=[0:max(0,len(faces)-1)])p2n(faces[i])]),
d= fp_tricylceb(faces,vertex),
c= fp_segorder(d),
e=fp_segcollapse(c),
g=shuffle(e,alt-round(rnd(2))),
f=fp_triangulate(g,n)
...
function fp_triangulate(a,n ,count=0)=
count>len(a)?
a
:
len(a)<=3?
[[a[0],a[1],a[2]]]:
sign(dot(p2n([a[0],a[1],a[2]]),n))>0?
let(b=concat([for(i=[2:len(a)-1])a[i]],[a[0]] )) 
concat([[a[0],a[1],a[2]]],fp_triangulate( shuffle(b,2),n )):fp_triangulate( shuffle(a,1),n,count+1 );
