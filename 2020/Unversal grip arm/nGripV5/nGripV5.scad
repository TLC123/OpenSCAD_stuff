...
union(){ 
   k=21;
    for(j=[ 95])
    for(i=[0:30:360]){
    translate  ([L*0.5 ,0.5])rotate(j)rotate([i,0,0])rotate([0,k,0]) translate([-B,0])rotate(90) 
        
         scale([.7,1.2,2]) sphere(lh*1.5,$fn=7); 
  ...
  module lock2d(){
   ...
   rotate(40)  union(){
      ...
      module lh(){
          ...
          a=[
          [-T*0.25,lh*6,lh*1.5],[T*-0.13,lh*9,lh*.9 ],
          [T*0.4,lh*14,lh*.9 ],
          
          [T*.4,(M-T*1.4)/2,lh*1. ],[T*.5,M/2,lh*2],];
 polyline(a) ;
