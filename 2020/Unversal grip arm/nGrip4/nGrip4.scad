...
translate([-(L/2-B*1.27),0,0]) square([lh,T/2],center=true); // live hinge
 ...
 linear_extrude(EX-   ch*2,center=true,convexity=50)  lock2d();
   }
        mirror([0,1])  hull(){
 
  linear_extrude(EX,center=true,convexity=50) offset(-ch,$fn=20)     lock2d();
  }
 ...
 rotate([90,0,0]) linear_extrude(M,center=true,convexity=50)
  ...
  module lock2d(){
   ...
   tumbler();
      ...
      module lh(){
         {
translate( [(M-T*1.0)/2*sin(-40), 0]) square([lh*2,T*0.4],center=true); // live hinge
          
          
                  translate([-T*0.5,-(M-T)/2+lh*0.25])    square([lh,T/2],center=true); // live hinge

     
     mirror([0,1])translate([-T*0.5,-(M-T)/2+lh*0.25])    square([lh,T/2],center=true); // live hinge
  ...
  translate([-T*0.5,-(M-T)/2+lh*0.25])   
      rotate(40.)  scale([1,0.95]) polygon([
      [0,0],
      [T-.5,0],
      [T,1],
      [T*0.66,(M-T*.76)/2],
