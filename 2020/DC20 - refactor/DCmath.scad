...
function Transf(p, M,f) = f(t( [p.x, p.y, p.z, 1] *  invtransform(p, M[0], M[1], [1,1,1])));

function invtransform(p, T = [0, 0, 0], R = [30, 30, 30], S = [1, 1,1]) =
let(S=is_undef(S)?[1,1,1]:S)
m_translate(T * -1) * m_rotate(R * -1)* m_scale(1/S);
