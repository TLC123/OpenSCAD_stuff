...
include<DCparser_eval.scad>
 ...
 scenecell= autobound(scenegraph,cubic=true,pad=0.02);
s=(scenecell[1].x-scenecell[0].x)/10;
showcell(scenecell);
echo(scenecell);


for(x=[scenecell[0].x:s:scenecell[1].x],
    y=[scenecell[0].y:s:scenecell[1].y],
    z=[scenecell[0].z:s:scenecell[1].z]
){
 p= [x,y,z];  
    translate(p) if (scenegraph(p)<=0)cube(s,center=true);
}
