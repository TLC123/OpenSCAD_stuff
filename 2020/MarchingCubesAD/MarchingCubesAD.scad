...
////////////// DEMO ////////////////////////////////////////////////////////////
move=rands(-5,5,3);
sdScene= function(p)    let(p=p-move)  min( (norm(p-[0,0,1])-2),  max(abs(p.x)-5,abs(p.y)-1,abs(p.z)-1 )) ;
...
$boundingBoxPadding=00.1;
$cubicBound=false;
sdDemo= true;
$fastBailout=0.87;
if(sdDemo){
    echo("Demo: sdDemo= false; to turn of demo");
    echo(" sdMarchingCubes() Usage: sdMarchingCubes( sdScene = f ( [ x , y , z ] ) , sub = subdivisions ) ");sdMarchingCubes(sdScene  ,sub=8);
...
//sdMC is a Octree subdivision MarchingCubes with direct gemoetry output
 module sdMarchingCubes(sdScene, cell=[],sub,parentCrv=0 ){
     assert(!is_undef(sub)&&!is_undef(sdScene),
     " sdMarchingCubes() Usage: sdMarchingCubes( sdScene = f ( [ x , y , z ] ) , sub = subdivisions ) ")
if (cell==[]) sdMarchingCubes(sdScene,autoBound(sdScene,cubic=$cubicBound,pad=$boundingBoxPadding ),sub ,parentCrv); else{
    O = cell[0]; S = cell[1];  C = (O + S) / 2; D = S - O;
maxD = max( abs (D.x),abs (D.y),abs (D.z));
Crv=abs(curvOf(sdScene,cell))/norm(D);
// { echo(Crv,sdScene(C),sub,cell,Crv);}
     if (sub>0&&  Crv*7>=parentCrv && abs(sdScene(C))<=maxD*$fastBailout )//ignore completly empty or full cells
    {       
            // subdivide cell into eight  smaller if subdivisions left 
//                 c=findTrueZero(C,evalnorm(C,sdScene), sdScene );
                    ...
                    for(i=[0:7]){sdMarchingCubes(sdScene,bflip([C,pxxx[i]]),sub-1, max(parentCrv,Crv) );  }    } 
         
       
      else       
            {
            // Make polyhedron of this cell
             p=vertFromCell  (cell) ;
          ...
          color([(Crv*2)%1,1,.5])    polyhedron(edgepoints,faces); } 
