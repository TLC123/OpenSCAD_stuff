...
function isedgeopen(i,edges)=revedgeof (i,edges)==[];

function isedgebrother(i,edges,points)= whatsibling(points[firstpointof(i,edges)])>whatsibling(points[lastpointof(i,edges)]) ;
function whatsibling(p)=p.x+p.y+p.z;

function ispointopen(i,edges,open)= 
 max([for(j=open) 
    let(jp1=firstpointof(j,edges),
        jp2=lastpointof(j,edges),
        ip1=firstpointof(i,edges),
        ip2=lastpointof(i,edges))
 (ip1==jp1||ip1==jp2)||(ip2==jp1||ip2==jp2)
 ]);
...
function sizeofrev(i,points,faces,edges)=  ifacesize( rightfaceof (revedgeof   (i,edges),edges) ,points,faces) ;
...
function isflatedge(i,points,faces,edges,neighbours,curvlim)=
let(
pA=firstpointof(i,edges),
pB=lastpointof(i,edges),
npA= (  [for(j=neighbours[pA]) ifacenormal(j,points,faces) ]),
npB= (  [for(j=neighbours[pB]) ifacenormal(j,points,faces) ]),
res=curvlim<= dot(un(addl( npA)),un(addl(npB)))
//,e=echo(pA,pB,npA,npB,res,neighbours[pA],neighbours[pB])

)res;
    
    
function isfoldedge(i,points,faces,edges,neighbours,curvlim)=
let(
pA=firstpointof(i,edges),
pB=lastpointof(i,edges),
fA=rightfaceof(i,edges),
fB=leftfaceof(i,edges),
nfA=ifacenormal(fA,points,faces) ,
nfB=ifacenormal(fB,points,faces) ,
nfABIsflat=abs(dot(nfA,nfB))>=flatness,
neighboursAB=unique(concat(neighbours[pA],neighbours[pB])),

dotnpAB=   [for(j=neighboursAB) let(i= ifacenormal(j,points,faces)) [ abs(dot(i,nfA))>=flatness,abs(dot(i,nfB))>=flatness]  ],
flatOrAB=   min([for(j=dotnpAB)  j[0]||j[1] ]),  // min of true/false pair list
//flatAndABis2orMore=   addl([for(j=dotnpAB)  j[0]&&j[1]?1:0 ])>=2,  // 


res=flatOrAB//&&(nfABIsflat||flatAndABis2orMore)
...
//midpoints=[for(i=remove)pconstruct(points[firstpointof(i,meshedges)] ,points[ lastpointof(i,meshedges)],scene)],
midpoints=[for(i=remove)
fitsinglepoint(   lerp(points[firstpointof(i,meshedges)] ,points[ lastpointof(i,meshedges)],0.5),scene )
],
...
let(
out=echo("Building Half edge structure"),
...
;
 ...
 function   Lsort(edges,meshedges,points)=
let (
    ledges=[for(i=edges)[lenghtof(i,points,edges),i]],
   sledges=lsort(ledges),
   vedges=[for(i=sledges)i[1]])
       vedges;
   
  function lsort(arr) = !(len(arr) > 0) ? [] :
 ...
 concat(lsort(lesser), equal, lsort(greater));  
...
function oneway(meshedges,testedges,  c=0,collect=[] )=c<len(testedges)?

let(
i=testedges[c],
opposing=revedgeof(i,meshedges),
     
notfound=(search(opposing,collect,1)==[]),
ncollect=notfound?concat(collect,[i]):collect)

oneway(meshedges,testedges,c+1,ncollect)
:
collect;    
