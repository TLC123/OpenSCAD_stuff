//Merges coplanar polyhedron tri faces to larger mostly flat polygons then let 
// OpenSCAD deal with the re-triangulation
...
// group coplanar faces of polyhedron into sub patches
  ...
  echo(mesh[1]);
for(lo=mesh[1]){color(rands(0,1,3))polyhedron(mesh[0],[lo]);                           }
...
points=mesh[0],
 ...
 newfaces=[ for(g=[0:len(m)-1])
     let(
        n=m[g],
        edges=edges(n),
        faces=n[1] ,
        edgesgroupid=unique(filteroutmax([for(i=[0:len(edges)-1])min(getedgeloop(i,edges))],len(edges)+1)))
        edgesgroupid==[]? []: [for(j=edgesgroupid) topointsindex(getedgeloop(j,edges),edges)] 
        ]
 ) 

 [points,flatten(newfaces)];
