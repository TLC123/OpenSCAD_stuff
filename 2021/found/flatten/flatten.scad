...
module test()
{
p=[for(i=[0:5])[rnd(0,100),rnd(0,100) ]];
po=[for(i=[0:len(p)-1])[p[i].x,p[i].y]];
pr=let(Lp1=len(p)-1)[for(i=[0:Lp1])min(10,  0.125*  spread(p[ wrap(i-1,Lp1+1)],p[i],p[ wrap(i+1,Lp1+1)]))];
R=polyRound(po,pr,5);
 polyline(R); 
%polyline(po);
echo([po]);
...
function dot (a,b)=a*b; // dotproduct is impied in * of vextoes
