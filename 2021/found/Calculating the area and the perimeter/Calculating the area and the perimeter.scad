...
function Perimeter_Of_A_Isosceles_Triangle_By_Base_And_Height(A,h)=
    A*sqrt((a/2)*(a/2)+h*h);
...
function Perimeter_Of_A_Regular_Polygon_By_Inradius_And_N(r,N)=   2*N*r*tan(180/N);
 function Perimeter_Of_A_Irregular_Polygon_By_Points(points, i=1) =let(    Side = (norm(points[i]-points[i-1])))
...
function Area_Of_A_Regular_Polygon_By_Side_And_N(A,N)= A*A*N/(4*tan(180/N));
