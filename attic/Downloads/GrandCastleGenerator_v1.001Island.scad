...
perimiter_sides = 6; // [4,5,6,8,12,60]
...
Island ( perimiter_diameter+max_turret_width*0.2, perimiter_height*1 ,perimiter_sides);



 
module Island(r,h,sides=4){
intersection(){translate([0,0,-h])cylinder(r,r,h*2);
...
translate([0,0,-h*2.2])scale([r,r,h*2.6])
...
color("DarkGreen")hull(){
  translate([0,0,-h*0.05]) rotate([0,0,ps])linear_extrude(1, 0, convexity = 3) offset(r = max_turret_width/3) {
    ...
    circle(r/2,$fn=sides);};
 translate([0,0,-h*2])rotate([0,0,rands(0,360,1)[0]])linear_extrude(1, 0, convexity = 3) offset(r = 4) {
      square(20, center = true);
    circle(r,$fn=sides /2);};
 translate([0,0,-h*2])rotate([0,0,rands(0,360,1)[0]])linear_extrude(1, 0, convexity = 3) offset(r = 4) {
      square(20, center = true);
...
translate([0,0,-h*2])scale([r,r,h*2.2])
polyhedron([for(i=[0:l])let(r=rands(0,360,1)[0],t=rands(90,180,1)[0])[sin(r)*cos(t),cos(r)*cos(t),sin(t)]],[for(j=[0:n])[rands(0,l-1,1)[0],rands(0,l-1,1)[0],rands(0,l-1,1)[0]]]);};
}   
      color("Grey") translate ([0,0,1.8])rotate([0,0,0])turret_round ( perimiter_diameter+max_turret_width*1.5, 200, false, perimiter_sides,0,0); }}}
