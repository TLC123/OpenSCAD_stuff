...
bon=bound(field,40,20);
quickuglymarch(field,bon);
 showfield(field,bon);
showbon(bon);
echo(bon); 
...
function opM(q,p,v)=pow(addl([for(i=[0:len(v)-1]) pow(eval(q,v[i]),2) ]),1/2);
...
opcode== opM ? opM(q,p,v[subra]): // 0 MetaBalls
...
T8=[ixmin ,iymin+S.y,izmin+S.z] ; quickuglymarch( ield,[T8,T8+S,step],d-1);
}
