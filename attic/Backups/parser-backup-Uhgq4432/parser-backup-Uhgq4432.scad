...
function opH(q,p,v)=let(vp=[opU,0,[for(i=[0:len(v)-1]) v[i]]],n=evalnorm(q,vp),e=eval(q,vp))e<0?e: 
 Hulltrace(q,q+un(n),vp)
 ;
...
function Hulltrace(q,qp,vp)=
let(n=evalnorm(qp,vp),m=eval(qp,vp),err=10000-len3(q-qp))

abs(err)<1?len3(q-qp)-m:  // if at target give diffrence
len3(n)<0.001?-m:   //stagnation in local minimum
abs(err)<m?   //
Hulltrace(q,q+un(n)*err*0.5,vp)
:
Hulltrace(q,q+un(n)*m,vp)
;
