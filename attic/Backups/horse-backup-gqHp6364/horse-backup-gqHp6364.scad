...
module blendline(v1, v2, s1 = 1, s2 = 1) {
   ...
   for (i = [0 + d: d: 1]) {
      hull() {
         translate(lerp(v1, v2, i)) scale(lerp(s1, s2, lerp(i,SC3(i),0.5))) rotate([0,0,360/16])ssphere(1);
         translate(lerp(v1, v2, i - d)) scale(lerp(s1, s2, lerp(i - d,SC3(i - d),0.5))) rotate([0,0,360/16]) ssphere(1);
      }
   }
}
module ssphere()
...
sphere([0.25,0.25,1]);
for(r=[0:360/$fn:360])
rotate([0,0,r])translate([1,0,0.25])sphere(0.1);
rotate([0,0,r])translate([1,0,-0.25])sphere(0.1);
}
