...
p6=  (p1+[S,0,S]),p7=  (p1+[0,S,S]),p8=  (p1+[S,S,S]),
...
pl=[for(i=[0:len(pl2)-1])if(pl2[i][1]!=[undef])pl2[i]], //filter
po=findp2p(p1+[s,s,s],pl,p1+[s,s,s]-[s,s,s]*1.25    ,p1+[s,s,s]+[s,s,s]*1.25 )  // feature detector
//po = push(po1)
)
...
function push(p,c=5)=let(q=evalnorm(p),np= p+un([q.x,q.y,q.z])*  (-q[3] )) c>0?push(np,c-1):np ; 
...
//Main evaluator
function eval(p,model )= min(
cube(invtransform(p),10),
cube(invtransform(p,[-20,0,0],[14,65,210],[1,1.2,0.9]),10),
...
module trender2(t){C0=(un(rndc())+[2,0,0]);C1=(un(rndc())+[2,2,0]);C2=(un(rndc())+[0,2,2]);for(i=[0:max(0,len(t[1])-1)]){ n=un(p2n(t[0][t[1][i][0]],t[0][t[1][i][1]],t[0][t[1][i][2]]));color(rndc())polyhedron(t[0],[t[1][i]]);}}
//////////////////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////////////////
