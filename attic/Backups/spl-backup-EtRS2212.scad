    ...
    for (i = [start + detail: detail: stop]) {
     /* if ($children > 0) {
        ...
        hull() {
          translate(t(spl2(i, v))) 
//rotate(spl2euler(i, v)) 
scale(spl2(i, v)[3]) rotate([twist * i, 0, 0]) rotate([0, -90, 0]) sphere(1);
          translate(t(spl2(i - detail * gap, v))) 
//rotate(spl2euler(i - detail * gap, v)) 
scale(spl2(i - detail * gap, v)[3]) rotate([twist * (i - detail), 0, 0]) rotate([0, -90, 0]) sphere(1);
        }
      }
    }
  }
...
function spl2(v, i) =
let (stop = len3spl(v) * i) spl2tworker(v, stop);
