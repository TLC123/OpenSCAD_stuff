...
#translate( [0,-0,0]) unglymarch( autohull(model),autobound(model,cubic=false,0.1) ,c=6);
translate( [0,0,0])unglymarch( model,autobound(model,cubic=false,0.1) ,c=6);
...
if(eC<maxD*1.7 )
{
    if(eC<-maxD*1.7 ){translate(C)cube(D,center=true);}
 else if(c>0)
    {
 p1=O;p2=  ( [S.x,O.y,O.z]);
...
else {translate(C)cube(D,center=true);}
}
...
// -min(
//
//cube(invtransform(p,[0,-12,11],[43, 43,43 ]),8),
//
//-min(
//minR(
//sphere(invtransform(p,[0,0,10]),13),
//cube(invtransform(p,[-20,0,0],[43, 4,2 ],[1,1,1]),10),4),
// cube(invtransform(p,[20,0,0],[6, 2 ,43],[1,1,1]),10,4)
//)
//
//)
min(sphere(invtransform(p,[5,-20,-10]),8),cube(invtransform(p,[0,20,10]),8),sphere(invtransform(p,[10,0,20],[10,0,20]),3))
:
...
#translate([cell[0][0],cell[1][1],cell[1][2]])cube(s);
       ...
       function autobound(model, cubic=false,padding=0.05)  = let(
...
[[esd,esd,esd]-d*padding,[wnu,wnu,wnu]+d*padding ]
        :
let(d=[west,north,up]-[east,south, down])
[[east,south, down]-d*padding , [west,north,up]+d*padding ]
        ;
 function autohull(model)  =[for(i=[1:250 ]) let(
        n=un(rndc(-1,1)),v= findbound(n,model)        )[n,v*n,v]]
        ;
