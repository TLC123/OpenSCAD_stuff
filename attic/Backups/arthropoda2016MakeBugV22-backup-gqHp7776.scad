...
polyhedron(Points,[for(ii=[-5:len(T)-1])[min(len(T)-1,max(0,ii)),min(len(T)-1,max(0,ii+1)),min(len(T)-1,max(0,ii+5)),min(len(T)-1,max(0,ii+6))]]);
lT1=len(T)-1;
for(i=[0:max(0,lT1)]){
...
[3,[[20,4,8],[0,25,0],[7,8,9],grove,ring,tex],lerp(bna2(),bna(),0.1),fork ]],rnd(1,1));
...
[1,[[5,22,23],[0,20,0],[7,8,9],grove,ring,tex],lerp(eye1(),eye2(),rnd()),fork]],rnd(1,1)); 
