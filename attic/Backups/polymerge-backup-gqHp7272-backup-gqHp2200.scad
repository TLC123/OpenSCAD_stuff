...
//mesh=fp_flip(bunny());
 midmesh = tesselate(mesh);
 nm = tesselate(midmesh);
n=neighbours(nm);


newmesh=relax (nm[0],nm[1],n);
dmaesh=fp_decimate ( newmesh  );
translate ([30,0,0]) polyhedron(nm[0], nm[1]);
translate ([-30,0,0]) polyhedron(dmaesh[0], dmaesh[1]);
  polyhedron(newmesh[0], newmesh[1]);
  for (i = [0: len(dmaesh) - 1]) echo(dmaesh[i]);
 

function tesselate(m)=
 let (
...
:
m;
function flattness(m,i)=round(rnd(1));
