...
//     [23, 22, 21, 20]
 ...
 mesh=fp_flip(bunny());
 midmesh = tesselate(mesh,2);
 dmaesh=fp_decimate ( midmesh );
 nm = tesselate(dmaesh,0);
...
function fp_decimate (m,i=0,unchanged=0)=
let(points=m[0],faces=m[1] )
i<len(points)-1?
let(r=flattness(m,i) )r<=0.009? fp_decimate (fp_decimateinidex(m,i),i,unchanged=1) :fp_decimate (m,i+1,unchanged)
:
unchanged==1?
fp_decimate(m)
:m
...
function flattness(m,j)=

let(points=m[0],faces=m[1],ni=neighboursofi(m,j),
...
ne
;
function fp_decimateinidex(m,j)=
let(nn=notneighboursofi (m,j),ni=neighboursofi(m,j),points=m[0],faces=m[1])
let(
