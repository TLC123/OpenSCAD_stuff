...
ping = push(p1), // Snap vertecies to nearst zero boundary 
 pong=normeq(pl)<=0.5?ping:findp2p(ping,pl,p1+[s,s,s]-[s,s,s]*15    ,p1+[s,s,s]+[s,s,s]*15 ) ,  // feature detector
 po = pong// push(pong)
...
function push(p,c=6)=let(q=evalnorm(p),np= p+un([q.x,q.y,q.z])*  (-q[3] )*0.5) c>0?push(np,c-1):abs(q[3]<0.01)?p:np ; 
...
cube(invtransform(p,[-20,0,0],[14,65,210],[1,1,1]),10)
,6),
