...
color("Red")  translate ([0,0,z])linear_extrude(0.5, convexity = 5){
difference(){
 offset(r=1)offset(r=-2)offset(r=1) projection(cut = true)  translate ([0,0,-z])piece(W,G,I);
offset(r=-2)offset(r=1) projection(cut = true)  translate ([0,0,-z])piece(W,G,I);}
for(t=[0:35]){
difference(){
offset(r=-2.1-t)offset(r=1) projection(cut = true)  translate ([0,0,-z])piece(W,G,I);
offset(r=-3-t)offset(r=1) projection(cut = true)  translate ([0,0,-z])piece(W,G,I);
}}
 }
}
