...
field=field();
function field(d=2)=d>0?[opU,1,[for(i=[0:3])[round(rnd(1,3)),un(rndc(-1,1))*50,[field(d-1)]]]]:[[cR3,5]];

span=100;step=50;bon=[ [-span,-span,-span],[span,span,span],(span*2)/step];
uglymarch(field,bon);
echo(bon[2]);echo(field);
...
{
 hull(){
if (val[ix][iy][iz]<0)translate([ix,iy,iz])sphere(1);
if (val[ix+step][iy][iz]<0)translate([ix+step,iy,iz])sphere(1);
if (val[ix+step][iy+step][iz]<0)translate([ix+step,iy+step,iz])sphere(1);
if (val[ix][iy+step][iz]<0)translate([ix,iy+step,iz])sphere(1);

if (val[ix][iy][iz+step]<0)translate([ix,iy,iz+step])sphere(1);
if (val[ix+step][iy][iz+step]<0)translate([ix+step,iy,iz+step])sphere(1);
if (val[ix+step][iy+step][iz+step]<0)translate([ix+step,iy+step,iz+step])sphere(1);
if (val[ix][iy+step][iz+step]<0)translate([ix,iy+step,iz+step])sphere(1);
}
}
}
