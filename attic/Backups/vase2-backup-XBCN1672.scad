VaseSeed=871458381;//	
OrnamentSeed=45848553818310271;//	
GlobalScale=[1,1,1];//
BrushScale=[2,1,0.5];//
BrushRotation=15;//
/* [Hidden] */
...
magfield=[cos(i+t/3)*2,0,0];
ndir=dir*95+cross(dir,magfield)*0.1;// blend dirction with force ( nonscientific)
np=op+ndir;
hull(){
rotate([0,0,op[1]*60])translate(concat(0,bez2((1-op[2]),v)))rotate([0,90,0])brush();
rotate([0,0,np[1]*60])translate(concat(0,bez2((1-np[2]),v)))rotate([0,90,0])brush();
echo(op,np);
}
if(i<25 ){spiral(np,ndir,t,i+0.1);}
