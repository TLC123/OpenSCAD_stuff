VaseSeed=8714576589586781;//	
OrnamentSeed=4584566310271;//	
//swirls or doodles
Swoodle=0;//[1,0]
GlobalScale=[1,1,1];//
BrushScale=[2,1,0.5];//
...
OrnamentAdjust=0.05;//
OrnamentAdjustR=2;//
Oreps=round(rnd(1,2,8));//
OHeight=rnd(1,0.65,0.35);//
/* [Hidden] */
...
k1=[rnd(OrnamentSeed+556,-90,-30):rnd(OrnamentSeed+76,30,70):rnd(OrnamentSeed+546,90,30)];
k2=[rnd(OrnamentSeed+516,-90,-30):rnd(OrnamentSeed+716,30,70):rnd(OrnamentSeed+514,90,30)];
...
//translate([0,0,60.0001])cylinder(h=10,r=60);
}

module spiral(op,dir,t, i=0){
magfield=[cos(i*8 +t*5)*3.5,0,0];
ndir=dir*0.95+cross(dir,magfield)*0.05;// blend dirction with force ( nonscientific)
np=op+ndir;
hull(){
rotate([0,0,op[1]*90*OrnamentAdjustR])translate(concat(0,bez2(max(0.024,min(0.95,1-op[2])),v)))rotate([0,90,0])brush();
rotate([0,0,np[1]*90*OrnamentAdjustR])translate(concat(0,bez2(max(0.024,min(0.95,1-np[2])),v)))rotate([0,90,0])brush();
}
