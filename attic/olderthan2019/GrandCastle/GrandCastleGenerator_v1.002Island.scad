...
Island ( perimiter_diameter*rands(0.9,1.5,1)[0]+max_turret_width*
        rands(0.1,0.5,1)[0], 
    perimiter_height*rands(0.4,1.5,1)[0],
    rands(-5,5,1)[0],
    ...
    difference(){ //Island minus cortyard and gate
      ...
      union(){
       ...
       for(loop=[0:loops]){
                ...
                ri=rands(0,360,l,seed+loop); // NEED TO USE rseed
                t=rands(90,180,l,seed+loop+1);  // NEED TO USE rseed
                v21=rands(0,l-1,n*3+1,seed+loop);// NEED TO USE rseed
       ...
       polyhedron(v1, v2);
        ...
        union(){
            ...
            hull(){
                ...
                translate([rands(-perimiter_diameter ,perimiter_diameter,1,seed+1)[0],rands(-perimiter_diameter,0,1,seed+2)[0],-h*2])
                rotate([0,0,rands(0,360,1,seed)[0]])
                linear_extrude(1, 0, convexity = 3) 
                offset(r = r*0.3)
                circle(r*0.7,$fn=sides /2);
                
                translate([rands(-perimiter_diameter ,perimiter_diameter,1,seed-1)[0],
                            rands(-perimiter_diameter ,0,1,seed-2)[0],-h*2])
                rotate([0,0,rands(0,360,1,seed+1)[0]])
                linear_extrude(1, 0, convexity = 3) 
                     offset(r = r*0.7)
                circle(r*0.3,$fn=sides /2);
