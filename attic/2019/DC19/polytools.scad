...
//gluemesh=smoothmesh(glue(cubemesh[0],cubemesh[1]));
 glum= [
    ...
    [[0, 1, 2], [0, 2, 3], [4, 5, 1], [4, 1, 0], [5, 6, 2], [5, 2, 1], [6, 7, 3], [6, 3, 2], [7, 4, 0], [7, 0, 3], [7, 6, 5], [7, 5, 4]]];
    
    gluemesh= smoothmesh(glum );
echo("glum",len(glum[0]),len(glum[1]));
  echo("smoothmesh", gluemesh );
...
function glue(points,faces)=points==[]?[]:
...
neighbours==undef?smoothmesh(points,faces, c,pointneighbours (points,faces)):
    let ( t = relaxmesh(points, faces, neighbours, c))
    t;


function pncpfiltered( points,faces ,neighbours)=
 [for(p=[0:len(points)-1]) avrgp(unique([ for(f= neighbours[p] ,fp=faces[f]) 
     if(p!=fp)points[fp]  ]))
...
let (np=pncpfiltered( points,faces ,neighbours)) 
relaxmesh(np, faces, neighbours, c - 1):
...
//function relaxmesh(points, faces, neighbours, c = 1) = c >0 ?
//let(np=pointneighbourscenterpoint(    facemidpoints(points,faces),neighbours,points))
//relaxmesh(np, faces, neighbours, c - 1):
//
// [points, faces];
